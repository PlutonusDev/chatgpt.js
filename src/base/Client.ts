...
export default class ChatGPTClient extends EventEmitter {
    ...
    constructor(config: ClientConfig) {
        ...
        this._updateClearance().then(() => {
            this._updateAccessToken().then(async m => {
                this.emit("debug", m);
                this.emit("ready");
    ...
    _updateClearance() {
        return new Promise(async (res, rej) => {
            if(!this.tlsClient) this.tlsClient = await initCycleTLS();

            const cfResponse = await this.tlsClient("https://chat.openai.com/", {
                method: "get",
